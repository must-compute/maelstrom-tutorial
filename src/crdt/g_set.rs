...
#[serde(untagged)]
pub enum GSetBody {
    Maelstrom(maelstrom_generic::Body),
    Replicate(ReplicateMsgBody),
}

impl MessageBody for GSetBody {
    fn msg_id(&self) -> usize {
        match self {
            GSetBody::Maelstrom(body) => body.msg_id(),
            GSetBody::Replicate(body) => body.msg_id(),
        }
    }

    fn set_msg_id(&mut self, msg_id: usize) {
        match self {
            GSetBody::Maelstrom(ref mut body) => body.set_msg_id(msg_id),
            GSetBody::Replicate(ref mut body) => body.set_msg_id(msg_id),
        };
    }
}

#[derive(Serialize, Deserialize, Clone, Debug, PartialEq)]
pub struct ReplicateMsgBody {
    msg_id: usize,
}

impl MessageBody for ReplicateMsgBody {
    fn msg_id(&self) -> usize {
        self.msg_id
    }
    fn set_msg_id(&mut self, msg_id: usize) {
...
impl Node {
    ...
    pub async fn handle(&mut self, msg: &maelstrom_generic::Message<GSetBody>) {
        match &msg.body {
            GSetBody::Maelstrom(body) => match body {
            ...
            GSetBody::Replicate(replicate_body) => {
                todo!()
            }
        }
    }

    fn send(&self, dest: &str, body: &maelstrom_generic::Body) {
