...
pub struct Node {
    ...
    retry_tx: OnceLock<mpsc::Sender<RetryMessage>>, // TODO: Fix this
}
...
impl Node {
    ...
    pub async fn handle(&self, msg: &maelstrom::Message, tx: Sender<RetryMessage>) {
...
pub async fn run() {
    ...
    // let unacked: Arc<Vec<maelstrom::Message>> = Arc::new(Vec::new());

    unacked.clone();
    while let Some(message) = rx.recv().await {
        let unacked = unacked.clone();
        match message {
            RetryMessage::Retry(msg) => unacked.push(msg),
            RetryMessage::StopRetry(msg_id) => unacked.retain(|msg| msg.body.msg_id() != msg_id),
        }
    }
    tokio::spawn(async move {
        loop {
            unacked.iter().for_each(|msg| {
                println!("{}", serde_json::to_string(msg).unwrap());
            });
